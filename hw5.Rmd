...
sim_results = 
  rerun(100, sim_lin_regression(30, 2, 0)) %>% 
  bind_rows()

repeat_results = 
  tibble(beta1 = c(0, 1, 2, 3, 4, 5, 6)) %>% 
  mutate(
    output_list = map(.x = beta1, ~rerun(100,       
...
repeat_results %>% 
  ...
  mutate(reject_rate = reject_count/100) %>% 
...
We can see that there is a positive relationship that when effect size increases, the power also increases. Given the null hypothesis, if we reject beta_1 = 0, the probability that a sample may reject null is relatively small.
...
Since the effect size and the the power have the positive linear relationship, when we increase the effect size, we are more likely to reject the null. Then with the increase of the number of the sample value, the mean is approching the null. 
