...
sim_results = 
  rerun(1000, sim_lin_regression(30, 2, 0)) %>% 
  bind_rows()

repeat_results = 
  tibble(beta1 = c(0, 1, 2, 3, 4, 5, 6)) %>% 
  mutate(
    output_list = map(.x = beta1, ~rerun(1000,       
...
repeat_results %>% 
  mutate(reject = ifelse(p_value <= 0.05, "significant", "non-significant")) %>% 
...
ggplot() + 
  geom_point(aes(x = beta_1, y = avg_beta_hat_pop, color = "4"), 
             data = pop_avg, size = 4, alpha = .4) +
  geom_smooth(aes(x = beta_1, y = avg_beta_hat_pop, color = "4"), 
              data = pop_avg, alpha = .5) +
  geom_point(aes(x = beta_1, y = avg_beta_hat_sample, color = "2"), 
             data = samples_avg, size = 4, alpha = .4) +
  geom_smooth(aes(x = beta_1, y = avg_beta_hat_sample, color = "2"), 
